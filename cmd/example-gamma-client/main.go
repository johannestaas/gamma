...
func cardPullerTool(args map[string]any) gamma.ToolResult {
...
func main() {
	...
	slog.Info("testing gamma client from CLI",
		"root_url", *rootURL,
		"model", *model,
		"prompt", *prompt,
	)
	...
	for {
		...
		if err != nil {
			if err == quitError {
				...
				slog.Error("panicking due to error", "error", err)
				panic(err)
			}
		}
		ans := convo.Ask(ctx, q)
		...
		for chunk := range ans {
			if chunk.Err != nil {
				slog.Error("chat error", "chunk", chunk)
				continue
			}
			if chunk.Thinking != "" {
